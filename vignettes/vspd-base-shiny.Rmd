...
- `ac_cycle_lnd`: A `SpatialPointsDataFrame` from the 'Stats19' dataset on road traffic collisions reported to the police.
- `commute_lines_lnd`: A `SpatialLinesDataFrame` representing commuter desire lines in London.
- `rf_lnd`: Lines with the same start and end point as `commute_lines_lnd` but alocated to the road network.
- `rnet_lnd`: A network of lines representing the cycling potential in different parts of London.
...
ac_cycle_lnd = readRDS("vspd-data/ac_cycle_lnd.Rds")
killed = ac_cycle_lnd[ac_cycle_lnd$Casualty_Severity == "Fatal",]
commute_lines_lnd = readRDS("vspd-data/l-lnd.Rds")
rf_lnd = readRDS("vspd-data/rf-lnd.Rds")
rnet_lnd = readRDS("vspd-data/rnet-lnd.Rds")
# lnd_msoas = readRDS("vspd-data/...")
# lnd_topo =  readRDS("vspd-data/...")
```
...
```{r}
plot(ac_cycle_lnd, pch = 1, axes = T)
```
...
```{r}
plot(ac_cycle_lnd, col = "yellow")
plot(lnd, add = T)
plot(rnet_lnd, add = T, col = "blue")
plot(ac_cycle_lnd[ac_cycle_lnd$Casualty_Severity == "Fatal",],
     col = "red", add = T)
```
...
```{r}
qtm(ac_cycle_lnd, dot.col = "yellow") +
  tm_shape(lnd) + tm_borders() +
  tm_shape(rnet_lnd) + tm_lines("blue") +
...
```{r, echo=FALSE}
tm_shape(rnet_lnd) + tm_lines(col = "grey") +
  tm_shape(killed) +
...
```{r}
tm_shape(killed) +
  tm_dots(col = c("Age_of_Casualty", "Sex_of_Casualty"), size = 0.1)
```
...
u2 = "https://github.com/npct/pct-data/raw/master/london/rf.Rds"
download.file(u2, "vspd-data/rf-lnd.Rds")
...
ac_cycle_lnd = ac_cycle_sp[lnd,]
plot(ac_cycle_lnd) # nearly 40k cyclist casualties
object.size(ac_cycle_lnd) / 1000000
names(ac_cycle_lnd)
# ac_cycle_lnd$Age_of_Casualty[ac_cycle_lnd$Age_of_Casualty == -1]
ac_cycle_lnd$Age_Band_of_Driver[is.na(ac_cycle_lnd$Age_Band_of_Driver)] = factor(NA)
levels(ac_cycle_lnd$Age_Band_of_Casualty)
ac_cycle_lnd@data = select(ac_cycle_lnd@data,
                           Accident_Index,
                           Date,
                           Day_of_Week,
                           Time,
...
saveRDS(ac_cycle_lnd, "vspd-data/ac_cycle_lnd.Rds")
rnet_lnd = readRDS("vspd-data/rnet-lnd.Rds")
summary(rnet_lnd$dutch_slc)
rnet_lnd = rnet_lnd[rnet_lnd$dutch_slc > 500,]
saveRDS(rnet_lnd, "vspd-data/rnet-lnd.Rds")
```
