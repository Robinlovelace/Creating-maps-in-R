...
recommend users are already acquaint themselves with the R command line
before beginning the practicals though, perhaps by following an
`Introduction to R' type tutorial, such as ``A (very) short introduction
to R'' (Torfs and Brauer, 2012) or the more geographically inclined
``Short introduction to R'' (Harris, 2012).
...
be downloaded from \url{http://cran.r-project.org/}. A number of R
editors such as \href{http://www.rstudio.com/}{RStudio} can be used to
make R more user friendly, but these are not needed to complete the
tutorial.
...
\NormalTok{sport <-}\StringTok{ }\KeywordTok{readShapePoly}\NormalTok{(}\StringTok{"london_sport.shp"}\NormalTok{)  }\CommentTok{# read in the shapefile}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{sport <-}\StringTok{ }\KeywordTok{readOGR}\NormalTok{(}\DataTypeTok{dsn =} \StringTok{"."}\NormalTok{, }\StringTok{"london_sport"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Map <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(sport.f, }\KeywordTok{aes}\NormalTok{(long, lat, }\DataTypeTok{group =} \NormalTok{group, }\DataTypeTok{fill =} \NormalTok{Partic_Per)) +}\StringTok{ }\KeywordTok{geom_polygon}\NormalTok{() +}\StringTok{ }
\StringTok{    }\KeywordTok{coord_equal}\NormalTok{() +}\StringTok{ }\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Easting (m)"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Northing (m)"}\NormalTok{, }\DataTypeTok{fill =} \StringTok{"% Sport Partic."}\NormalTok{) +}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{b <-}\StringTok{ }\KeywordTok{bbox}\NormalTok{(sport.wgs84)}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(rgdal)}
\NormalTok{lnd <-}\StringTok{ }\KeywordTok{readOGR}\NormalTok{(}\DataTypeTok{dsn =} \StringTok{"."}\NormalTok{, }\StringTok{"london_sport"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{crimeDat <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"mps-recordedcrime-borough.csv"}\NormalTok{)  }\CommentTok{# flags an error}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{crimeDat <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"mps-recordedcrime-borough.csv"}\NormalTok{, }\DataTypeTok{fileEncoding =} \StringTok{"UCS-2LE"}\NormalTok{)}
\KeywordTok{head}\NormalTok{(crimeDat)}
\KeywordTok{summary}\NormalTok{(crimeDat$MajorText)}
\NormalTok{crimeTheft <-}\StringTok{ }\NormalTok{crimeDat[}\KeywordTok{which}\NormalTok{(crimeDat$MajorText ==}\StringTok{ "Theft & Handling"}\NormalTok{), ]}
\KeywordTok{head}\NormalTok{(crimeTheft, }\DecValTok{2}\NormalTok{)  }\CommentTok{# change 2 for more rows}
\NormalTok{crimeAg <-}\StringTok{ }\KeywordTok{aggregate}\NormalTok{(CrimeCount ~}\StringTok{ }\NormalTok{Spatial_DistrictName, }\DataTypeTok{FUN =} \StringTok{"sum"}\NormalTok{, }\DataTypeTok{data =} \NormalTok{crimeTheft)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lnd$name %in%}\StringTok{ }\NormalTok{crimeAg$Spatial_DistrictName}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lnd$name[}\KeywordTok{which}\NormalTok{(!lnd$name %in%}\StringTok{ }\NormalTok{crimeAg$Spatial_DistrictName)]}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{levels}\NormalTok{(crimeAg$Spatial_DistrictName)[}\DecValTok{25}\NormalTok{] <-}\StringTok{ }\KeywordTok{as.character}\NormalTok{(lnd$name[}\KeywordTok{which}\NormalTok{(!lnd$name %in%}\StringTok{ }
\StringTok{    }\NormalTok{crimeAg$Spatial_DistrictName)])}
\NormalTok{lnd$name %in%}\StringTok{ }\NormalTok{crimeAg$Spatial_DistrictName  }\CommentTok{# now all columns match}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{crimeAg <-}\StringTok{ }\KeywordTok{rename}\NormalTok{(crimeAg, }\DataTypeTok{replace =} \KeywordTok{c}\NormalTok{(}\DataTypeTok{Spatial_DistrictName =} \StringTok{"name"}\NormalTok{))}
...
\KeywordTok{library}\NormalTok{(rgdal)}
\NormalTok{stations <-}\StringTok{ }\KeywordTok{readOGR}\NormalTok{(}\DataTypeTok{dsn =} \StringTok{"."}\NormalTok{, }\DataTypeTok{layer =} \StringTok{"lnd-stns"}\NormalTok{, }\DataTypeTok{p4s =} \StringTok{"+init=epsg:27700"}\NormalTok{)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stationsWGS <-}\StringTok{ }\KeywordTok{spTransform}\NormalTok{(stations, }\DataTypeTok{CRSobj =} \KeywordTok{CRS}\NormalTok{(}\KeywordTok{proj4string}\NormalTok{(lnd)))}
\NormalTok{stations <-}\StringTok{ }\NormalTok{stationsWGS}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{int <-}\StringTok{ }\KeywordTok{gIntersects}\NormalTok{(stations, lnd, }\DataTypeTok{byid =} \NormalTok{T)  }\CommentTok{# find which stations intersect }
...
\KeywordTok{which}\NormalTok{(int[, }\DecValTok{500}\NormalTok{] ==}\StringTok{ }\NormalTok{T)  }\CommentTok{# this tells us that point 500 intersects with zone 32}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{clipped <-}\StringTok{ }\KeywordTok{apply}\NormalTok{(int ==}\StringTok{ }\NormalTok{F, }\DataTypeTok{MARGIN =} \DecValTok{2}\NormalTok{, all)}
\KeywordTok{plot}\NormalTok{(stations[}\KeywordTok{which}\NormalTok{(clipped), ])  }\CommentTok{# shows all stations we DO NOT want}
\NormalTok{stations.cl <-}\StringTok{ }\NormalTok{stations[}\KeywordTok{which}\NormalTok{(!clipped), ]  }\CommentTok{# use ! to select the invers}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stations <-}\StringTok{ }\NormalTok{stations.cl}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{int <-}\StringTok{ }\KeywordTok{gIntersects}\NormalTok{(stations, lnd, }\DataTypeTok{byid =} \NormalTok{T)  }\CommentTok{# re-run the intersection query }
...
\NormalTok{b.indexes <-}\StringTok{ }\KeywordTok{which}\NormalTok{(int, }\DataTypeTok{arr.ind =} \NormalTok{T)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{b.count <-}\StringTok{ }\KeywordTok{rename}\NormalTok{(b.count, }\DataTypeTok{replace =} \KeywordTok{c}\NormalTok{(}\DataTypeTok{b.names =} \StringTok{"name"}\NormalTok{))}
\NormalTok{b.count.tmp <-}\StringTok{ }\KeywordTok{join}\NormalTok{(lnd@data, b.count)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lnd$station.count <-}\StringTok{ }\NormalTok{b.count.tmp[, }\DecValTok{7}\NormalTok{]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{input <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"ambulance_assault.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.ass <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(input, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{assault_09_11))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.ass +}\StringTok{ }\KeywordTok{geom_histogram}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.ass +}\StringTok{ }\KeywordTok{geom_histogram}\NormalTok{(}\DataTypeTok{binwidth =} \DecValTok{10}\NormalTok{) +}\StringTok{ }\KeywordTok{geom_density}\NormalTok{(}\DataTypeTok{fill =} \OtherTok{NA}\NormalTok{, }\DataTypeTok{colour =} \StringTok{"black"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p2.ass <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(input, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{assault_09_11, }\DataTypeTok{y =} \NormalTok{..density..))}

\NormalTok{p2.ass +}\StringTok{ }\KeywordTok{geom_histogram}\NormalTok{() +}\StringTok{ }\KeywordTok{geom_density}\NormalTok{(}\DataTypeTok{fill =} \OtherTok{NA}\NormalTok{, }\DataTypeTok{colour =} \StringTok{"red"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{input[}\KeywordTok{which}\NormalTok{(input$assault_09_11 >}\StringTok{ }\DecValTok{750}\NormalTok{), ]}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p3.ass <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(input, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \NormalTok{Bor_Code, }\DataTypeTok{y =} \NormalTok{assault_09_11))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p3.ass +}\StringTok{ }\KeywordTok{geom_boxplot}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p3.ass +}\StringTok{ }\KeywordTok{geom_boxplot}\NormalTok{() +}\StringTok{ }\KeywordTok{coord_flip}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.ass +}\StringTok{ }\KeywordTok{geom_histogram}\NormalTok{() +}\StringTok{ }\KeywordTok{facet_wrap}\NormalTok{(~Bor_Code)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{london.data <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"census-historic-population-borough.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{plot.data <-}\StringTok{ }\NormalTok{plot.data[}\KeywordTok{order}\NormalTok{(plot.data$order), ]}
\end{Highlighting}
\end{Shaded}
