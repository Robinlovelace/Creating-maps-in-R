...
\NormalTok{crime_ag <-}\StringTok{ }\KeywordTok{aggregate}\NormalTok{(CrimeCount ~}\StringTok{ }\NormalTok{DName, }\DataTypeTok{FUN =} \NormalTok{sum, }\DataTypeTok{data =} \NormalTok{crime_theft)}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Compare the name column in lnd to DName column in crime_ag to see which rows match.}
\NormalTok{lnd$name %in%}\StringTok{ }\NormalTok{crime_ag$DName}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{lnd$name[!lnd$name %in%}\StringTok{ }\NormalTok{crime_ag$DName]}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{levels}\NormalTok{(crime_ag$DName)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{sel <-}\StringTok{ }\NormalTok{!lnd$name %in%}\StringTok{ }\NormalTok{crime_ag$DName }\CommentTok{# create selection}
\CommentTok{# Rename row 25 in crime_ag to match row 25 in lnd, as suggested results form above}
\KeywordTok{levels}\NormalTok{(crime_ag$DName)[}\DecValTok{25}\NormalTok{] <-}\StringTok{ }\KeywordTok{as.character}\NormalTok{(lnd$name[sel])}
\KeywordTok{summary}\NormalTok{(lnd$name %in%}\StringTok{ }\NormalTok{crime_ag$DName) }\CommentTok{# now all columns match}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{head}\NormalTok{(crime_ag$DName) }\CommentTok{# the variables to join}
\NormalTok{crime_ag <-}\StringTok{ }\KeywordTok{rename}\NormalTok{(crime_ag, }\DataTypeTok{name =} \NormalTok{DName)}
