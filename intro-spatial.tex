...
\KeywordTok{plot}\NormalTok{(x, y)  }\CommentTok{# plot the result (not shown in tutorial)}
\end{Highlighting}
\end{Shaded}
...
these elements in this part of the tutorial, before focussing on
creating attractive maps in Part III.
...
\texttt{dsn} argument in this case, specifies the directory in which the
...
\texttt{head(sport@data)}), but sometimes this is a bit much, so we can
specify the number of lines with \texttt{n = 2} after the comma. The
second line of the code above calculates the mean value of the variable
\texttt{Partic\_Per} (sports participation per 100 people) for each of
the zones in the sport object. To explore the sport object further, try
typing \texttt{nrow(sport)} and record how many zones the dataset
contains.You can also try \texttt{ncol(sport)}.
...
its behaviour depending on the input data. Inputing another dataset
(e.g. \texttt{plot(sport@data))} will generate an entirely different
type of plot altogether. Thus R is intelligent at guessing what you want
...
\caption{plot of chunk Preliminary plot of London. Areas with high
...
objects are kept in a `slot' that can be accessed using the @ symbol:
\texttt{sport@data}. This is useful if you do not wish to work with the
spatial components of the data at all times.

Type \texttt{summary(sport)} to get some additional information about
the sport data object. Spatial objects in R contain a variety of
...
shapefile as a new object and plot it. We will call this new object
\texttt{lnd}, short for London:
...
a dataset on recorded crimes, this dataset currently resides in a CSV
file called ``mps-recordedcrime-borough'' with each row representing on
crime. We are going to use the non-spatial implementation of a function
called \texttt{aggregate} to pre-process this dataset ready to join to
our spatial \texttt{lnd} dataset. First we will create a new object
called \texttt{crimeDat} to store this data.

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# create new crimeDat object from crime data}
...
\CommentTok{# display first 6 lines of the crimeDat object}
\KeywordTok{head}\NormalTok{(crimeDat)}
\CommentTok{# summarise the column 'MajorText' for the crimeDat object}
\KeywordTok{summary}\NormalTok{(crimeDat$MajorText)}
\CommentTok{# Extract 'Theft & Handling' crimes from crimeDat object and save these as a}
\CommentTok{# new object named 'crimeTheft'}
\NormalTok{crimeTheft <- crimeDat[}\KeywordTok{which}\NormalTok{(crimeDat$MajorText == }\StringTok{"Theft & Handling"}\NormalTok{), ]}
\CommentTok{# change 2 for more rows}
\KeywordTok{head}\NormalTok{(crimeTheft, }\DecValTok{2}\NormalTok{)}
\CommentTok{# Calculate the sum of the crime count for each district. Aggregate the data}
\CommentTok{# and place in new object called crimeAg.}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# rename row 25 in crimeAg to match row 25 in lnd.}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{stations.c <- }\KeywordTok{aggregate}\NormalTok{(}\DataTypeTok{data =} \NormalTok{stations, }\DataTypeTok{by =} \NormalTok{lnd, }\DataTypeTok{FUN =} \NormalTok{length)}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{int <- }\KeywordTok{gIntersects}\NormalTok{(}\DataTypeTok{data =} \NormalTok{stations, }\DataTypeTok{by =} \NormalTok{lnd, }\DataTypeTok{byid =} \OtherTok{TRUE}\NormalTok{)  }\CommentTok{# re-run the intersection query }
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
...
\section{Part III map making with ggplot2}
...
\section{Part IV: Taking spatial data analysis in R further}
