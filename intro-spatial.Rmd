...
```{r Choropleth map of mean values of stations in each borough, fig.keep='none'}
...
```{r Symbol levels for train station types in London, echo=FALSE}
q <- cut(stations.m$NUMBER,  breaks= c(quantile(stations.m$NUMBER)), include.lowest=T)
clr <- as.character(factor(q, labels = paste0("grey", seq(20, 80, 20))))
...
```{r Transport points in Barking and Dagenham}
