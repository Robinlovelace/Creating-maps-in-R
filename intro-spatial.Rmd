...
```{r}
#select rows from attribute slot of sport object, where sports participation is less than 15.
...
```{r Preliminary plot of London. Areas with high sports participation are blue}
plot(sport)
...
#Extract "Theft & Handling" crimes from crimeDat object and save these as a new object named "crimeTheft"
crimeTheft <- crimeDat[which(crimeDat$MajorText == "Theft & Handling"), ] 
# change 2 for more rows
head(crimeTheft, 2) 
#Calculate the sum of the crime count for each district. Aggregate the data and place in new object called crimeAg.  
...
```{r}
#compare the name column in lnd to Spatial_DistrictName column in crimeAg to see which rows match.
...
stations <- stations27700; rm(stations27700) # remove the stations27700 object to clear up
plot(lnd)
points(stations)
```
...
```{r Choropleth map of mean values of stations in each borough}
...
```{r Basemap 3}
lnd.b3 <- ggmap(get_map(location = b, source = "stamen", maptype = "toner", crop = TRUE, zoom = 11))
...
# If not install it, or skip the next two stepsâ¦
```
