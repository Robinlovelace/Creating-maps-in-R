...
let us look at the object's *geometry* data, which describes where
...
Inputting another dataset such as `plot(lnd_sport@data)` will generate
...
So far we have been interrogating only the attribute *slot* (`@data`) of the `lnd_sport` object, but the square brackets can also be used to subset spatial datasets, i.e. the geometry *slot*. Using the same logic as before try
...
summary of the `lnd_sport` object above. This represents the coordinate reference system used in the data.
...
# Extract "Theft & Handling" crimes from crimeDat object and save these as crimeTheft
...
# Rename row 25 in crimeAg to match row 25 in lnd, as suggested results form above
levels(crimeAg$Spatial_DistrictName)[25] <-
  as.character(lnd$name[!lnd$name %in% crimeAg$Spatial_DistrictName])
...
```{r}
stations.m <- aggregate(stations_agg("NUMBER")] , by = lnd, FUN = mean)
```
...
```{r, warning=FALSE}
lnd.f <- fortify(lnd, region = "ons_label") # you may need to load maptools
```
...
Map <- ggplot(lnd.f, aes(long, lat, group = group, fill = partic_per)) +
  geom_polygon() +
  coord_equal() +
...
# save(lnd.b1, file = "data/lnd.b1.RData")
load("data/lnd.b1.RData") # load saved map
```
...
data (we already did this step to create the `lnd.f` object).

```{r}
lnd_sport_wgs84.f <- fortify(lnd_sport_wgs84, region = "ons_label")
lnd_sport_wgs84.f <- merge(lnd_sport_wgs84.f, lnd_sport_wgs84@data,
...
```{r, eval=FALSE}
lnd.b1 +
  geom_polygon(data = lnd_sport_wgs84.f,
...
```{r, eval=FALSE}
# download basemap (use load("data/lnd.b2.RData") if you have no internet)
lnd.b2 <- ggmap(get_map(location = b, source = "stamen",
          maptype = "toner", crop = TRUE))
```

```{r, echo=FALSE}
# save(lnd.b2, file = "data/lnd.b2.RData")
load("data/lnd.b2.RData") # load saved map
```
...
```{r fig.cap="Basemap 2"}
library(mapproj) # mapproj library needed here
...
```{r, eval=FALSE}
# download basemap (try load("data/lnd.b3.RData") if you lack internet)
lnd.b3 <- ggmap(get_map(location = b, source = "stamen",
  maptype = "toner", crop = TRUE, zoom = 11))
```

```{r, eval=F, echo=FALSE}
# save(lnd.b3, file = "data/lnd.b3.RData")
# load("data/lnd.b3.RData") # load saved map
```
...
```{r}
london_data_molten <- melt(london_data, id = c("Area.Code", "Area.Name"))
```


```{r, echo=FALSE, eval=FALSE}
# london_data_molten <- read.csv("london_data_melt.csv")
# Only do this step if reshape and melt failed
```

Merge the population data with the London borough geometry contained within our `lnd.f` object.

```{r}
plot.data <- merge(lnd.f, london_data_molten, by.x = "id", by.y = "Area.Code")
```
...
```{r, echo=FALSE}
# system("mv intro-spatial.pdf intro-spatial-rl.pdf")
```