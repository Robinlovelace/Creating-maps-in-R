...
```{r}
england = gUnaryUnion(lnd, lnd$dummy)
england = SpatialPolygonsDataFrame(england, data.frame(dummy = c("England")), match.ID = FALSE)

centreEngland = gCentroid(england, byid = TRUE)

centreLEP = gCentroid(lnd, byid = TRUE)
coords <- cbind(centreEngland$y)

c1 = c(centreEngland$x, centreEngland$x)
c2 = c(90, -90)
c3 = c(90, -90)
c4 = c(centreEngland$y,centreEngland$y)
...
mask <- gUnion(Longitude, Latitude, byid=FALSE, id=NULL, drop_lower_td=FALSE)

# plot(england)
# plot(centreEngland,col='red',add=TRUE)
# plot(centreLEP, col= "blue", add=TRUE)

# plot(mask, col = 'red', add =TRUE)

proj4string(mask) <- proj4string(england)

# intersect <- gIntersection(mask, england,  byid=FALSE, id=NULL, unaryUnion_if_byid_false=FALSE)

# plot(intersect)
# plot(england, add = TRUE)
...
plot(lnd[east & north,],add = TRUE, col = "red" )
 llgridlines(england, lty= 3, side ="WS", offset = 0.1)
