...
```{r, echo=FALSE, fig.cap="Tab-autocompletion after typing 'lnd@' to see which slots are contained within"}
...
plot(lnd[lnd$Partic_Per > 25, ]) # output not shown here
```
...
plot(lnd) # plot the london_sport object
sel <- lnd$Partic_Per > 25 # select the zones with high sports participation
plot(lnd[ sel , ], col = "blue", add = TRUE) # add selected zones to existing map
```
...
As a bonus stage, let's try to select only zones that are
close to the centre of London. Programming encourages rigorous
thinking, so we must first be more precise in our formulation
of the problem:

> **Challenge**: Select all zones whose geographic centroid lies within 10 km of the geographic centroid of inner London.

```{r, echo=FALSE, fig.cap="Zones in London whose centroid lie within 10 km of the geographic centroid of inner London"}
library(rgeos)
centlnd <- gCentroid(lnd)
cbuf <- gBuffer(spgeom = centlnd, width = 5000)
lnd_cent <- lnd[cbuf,]
plot(lnd, col = "grey")
plot(lnd_cent, col = "blue", add = T)
text(coordinates(centlnd), "Central London", col = "white")
...
can now plot the rate of theft crimes in London by borough 
(see Fig 8).
...
Using this dataset and the methods developed above, Figure 7 was
...
in [intro-spatial.Rmd]() on GitHub, at [github.com/Robinlovelace/Creating-maps-in-R/blob/master/intro-spatial.Rmd](https://github.com/Robinlovelace/Creating-maps-in-R/blob/master/intro-spatial.Rmd).]
...
plot(lnd) # plot London for context (see Figure 8)
...
plot(stations) # test the clip succeeded (see Figure 9)
```
...
borough (the basis for Figure 10). As an additional step, try comparing the mean
...
code to the chunk added above (see Figure 11):
