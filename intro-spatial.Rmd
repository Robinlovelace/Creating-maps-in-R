...
We also suggest that you use an R editor, such as [RStudio](http://www.rstudio.com/), for this tutorial. R editors improve the user-experience and help with the learning process.     
...
The spatial packages we will use today are:
...
Run the above commands to install these packages now.
...
If you ever need to change the working directory of R, you can use the 'Session' menu at the top of the page or use the [`setwd` command](http://www.statmethods.net/interface/workspace.html).
...
The documentation for join will be displayed if
the **dplyr** package is loaded (if not,
...
lnd@data <- inner_join(lnd@data, crime_ag)
```
...
Reorder this data (ordering is important for plots) and rename the dates.
...
```{r fig.cap="Faceted map", fig.height=7, fig.width=7}
ggplot(data = lnd_f,
  aes(x = long, y = lat, fill = pop/1000, group = group)) +
  geom_polygon() +
...
# ggsave("figure/facet_london.png", width = 9, height = 9)
```
