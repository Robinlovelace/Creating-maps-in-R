...
**ggmap** (which extends **ggplot**), **rgdal**, **rgeos**, **maptools** and **dplyr**.
...
Click on the "Download ZIP" button on the right hand side and once
it is downloaded unzip this to a new folder on your PC.

If you are working in RStudio, you can create a project that will automatically
set your working directory.
To do this click on `File -> Open File...` in the top
menu, navigate to unzipped `Creating-maps-in-R` folder and double click
on the `Creating-maps-in-R.Rproj` project file. Alternatively, you can use the *project menu* to open the project or create a new one. It is 
...
project or use the [`setwd` command](http://www.statmethods.net/interface/workspace.html).
...
crime_ag <- aggregate(CrimeCount ~ DName, FUN = sum, data = crime_theft)
...
Now that we have crime data at the borough level (`DName`), the challenge is to join it to the `lnd` object. We will base our join on the `DName` variable from the `crime_ag` object and the `name` variable from the `lnd` object. It is not always straight forward to join objects based on names as the names do not always match. Let us see which names in the `crime_ag` object match the spatial data object, `lnd`:

```{r}
# Compare the name column in lnd to DName column in crime_ag to see which rows match.
lnd$name %in% crime_ag$DName
# Return rows which do not match
lnd$name[!lnd$name %in% crime_ag$DName]
```
...
levels(crime_ag$DName)

# Select the problematic row - the one which *does not* (via the ! symbol) match 
sel <- !lnd$name %in% crime_ag$DName # create selection
# Rename row 25 in crime_ag to match row 25 in lnd, as suggested results form above
levels(crime_ag$DName)[25] <- as.character(lnd$name[sel])
summary(lnd$name %in% crime_ag$DName) # now all columns match
```
...
the `inner_join` function **dplyr** but the `merge` function
...
```{r, results='hide'}
help(inner_join) # error flagged
library(dplyr) # load the powerful dplyr package (use plyr if unavailable)
help(inner_join) # should now be loaded (use join if unavailable)
```
...
```{r, results='hide'}
head(lnd$name)
head(crime_ag$DName) # the variables to join
crime_ag <- rename(crime_ag, name = DName)
...
This step has lost the attribute information associated with the lnd object. We can add it back using the `inner_join`
function (this performs a data join).
To find out how this function works look at
the output of typing `?inner_join`.

```{r}
lnd_f <- inner_join(lnd_f, lnd@data, by = c("id" = "ons_label"))
```
...
lnd_wgs84_f <- inner_join(lnd_wgs84_f, lnd_sport_wgs84@data,
  by = c("id" = "ons_label"))
```
...
Merge the population data with the London borough geometry contained within our `lnd_f` object, using the `inner_join` function from
the **dplyr** package:

```{r}
ltidy <- rename(ltidy, id = Area.Code)
lnd_f <- inner_join(lnd_f, ltidy)
```
...
```{r fig.cap="Faceted map", fig.height=6, fig.width=5}
ggplot(data = lnd_f,
