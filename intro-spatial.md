...
plot(x, y)  # plot the result (not shown in tutorial)
```
...
set your working directory.To do this click "Session" from the top toolbar and select "Set working directory > choose directory". 
...
but sometimes this is a bit much, so we can specify the number of lines with `n = 2` after the comma. 
...
Remember, the attribute data contained in spatial objects are kept in a 'slot' that can be accessed using 
the @ symbol: `sport@data`. This is useful if you do not wish to work with the spatial components of the data at all times. 

Type `summary(sport)` to get some additional information about the sport data object. Spatial objects in R contain a variety of additional information:
...
"london_sport" shapefile as a new object and plot it. We will call this new object 
...
dataset on recorded crimes, this dataset currently resides in a CSV file called "mps-recordedcrime-borough" with each row representing on crime.
We are going to use the non-spatial implementation of a function called `aggregate` 
...
```r
# create new crimeDat object from crime data
crimeDat <- read.csv("data/mps-recordedcrime-borough.csv", fileEncoding = "UCS-2LE")
# display first 6 lines of the crimeDat object
head(crimeDat)
# summarise the column 'MajorText' for the crimeDat object
summary(crimeDat$MajorText)
# Extract 'Theft & Handling' crimes from crimeDat object and save these as a
# new object named 'crimeTheft'
crimeTheft <- crimeDat[which(crimeDat$MajorText == "Theft & Handling"), ]
# change 2 for more rows
head(crimeTheft, 2)
# Calculate the sum of the crime count for each district. Aggregate the data
# and place in new object called crimeAg.
...
```r
# rename row 25 in crimeAg to match row 25 in lnd.
